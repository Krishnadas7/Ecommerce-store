...
const placeOrder = async (req, res) => {
  try {
    ...
    const productIDs = cartData.products.map(
      ...
      productData.forEach(order => {
        if (order.products && order.products.length > 0) {
          order.products.forEach(product => {
            const price =
              product.productId.discountedAmount || product.productId.price
    ...
    if (orderdata) {
      if (paymentMethods == 'COD') {
        const minus = await Coupon.updateOne(
          { couponCode: req.session.code },
          { $inc: { usersLimit: -1 } }
        )
        const pushing = await Coupon.updateOne(
          { couponCode: req.session.code },
  ...
  try {
    ...
    if (hmacValue === details.payment.razorpay_signature) {
      ...
      await Order.findByIdAndUpdate(
        ...
        {
          $set: {
            'products.$[].paymentStatus': 'Completed',
...
const cancelOrder = async (req, res) => {
  try {
    ...
    if (order.paymentMethod != 'COD') {
      ...
      const totalInc = await Order.updateOne(
        { _id: orderId },
        { $inc: { totalAmount: -minusAmount } }
      )
